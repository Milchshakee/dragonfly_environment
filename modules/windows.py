...
class WinSelectorRule(CompoundRule):
    ...
    spec = "window | win | [window] <window_names>"
    extras = [window_names_ref]
    exported = False

    def value(self, node):
        if node.has_child_with_name("window_names"):
            return node.get_child_by_name("window_names").value()
...
class MonSelectorRule(CompoundRule):
    ...
    spec = "(this | current) monitor | [monitor] <monitor_names>"
    extras = [monitor_names_ref]
    exported = False

    def value(self, node):
        if node.has_child_with_name("monitor_names"):
            return node.get_child_by_name("monitor_names").value()
        return None
